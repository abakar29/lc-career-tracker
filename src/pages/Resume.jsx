          ...
          <div className="rounded-2xl border-l-2 border-l-brand-orange bg-white p-6 shadow-sm">
            <div className="flex items-start justify-between gap-2">
              ...
              <button
                ...
                className="inline-flex flex-shrink-0 items-center gap-1 text-xs font-semibold text-[#E87722] hover:opacity-80"
              >
                {quickEditMode ? (
                  "Done"
                ) : (
                ...
                {quickEditMode ? (
                  <>
                      ...
                      {expItems.map((item, i) => (
                        <li key={i} className="flex items-center gap-2">
                          ...
                          <input
                            ...
                            className="w-full rounded-md border border-slate-300 px-2 py-1 text-sm text-[#111827] focus:outline-none focus:ring-2 focus:ring-[#E87722]"
                          />
                          <button
                            type="button"
                            onClick={() => removeExpItem(i)}
                            aria-label="Remove experience"
                            className="flex-shrink-0 text-slate-400 hover:text-[#E87722]"
                          >
                    ...
                    <button
                      type="button"
                      onClick={addExpItem}
                      className="mt-2 text-xs font-semibold text-[#E87722] hover:opacity-80"
                    >
                      + Add Experience
                    </button>
                    ...
                    <div className="mt-4 flex flex-wrap items-center gap-2">
                      {skillItems.map((item, i) => (
                        <span
                          key={i}
                          className="inline-flex items-center gap-1 rounded-full border border-slate-300 bg-slate-100 px-2 py-0.5"
                        >
                          <input
                            type="text"
                            value={item}
                            ...
                            className="bg-transparent text-xs text-[#6B7280] focus:outline-none"
                          />
                          <button
                            type="button"
                            onClick={() => removeSkillItem(i)}
                            aria-label="Remove skill"
                            className="text-slate-400 hover:text-[#E87722]"
                          >
        ...
        <div className="mt-6 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {RESUME_TEMPLATES.map((template) => {
            ...
            return (
              ...
              >
                <div className="flex h-11 w-11 items-center justify-center rounded-full bg-[#FFF3E8]">
                  <Icon className="h-5 w-5 text-[#E87722]" aria-hidden="true" />
                </div>
                <h3 className="mt-4 text-sm font-bold text-[#111827]">{template.name}</h3>
                <p className="mt-1 flex-1 text-sm text-[#6B7280]">{template.description}</p>
                <button
                  type="button"
                  onClick={handleDownloadTemplate}
                  className="mt-5 rounded-lg border border-[#E87722] px-4 py-2.5 text-sm font-semibold text-[#B85A12] transition-colors hover:bg-[#FFF3E8]"
                >
                  Download Template
                </button>
              </div>
